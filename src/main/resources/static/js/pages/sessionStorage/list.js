// sessionStorage 게시글 목록 페이지 JavaScript
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayEmptyState() {
        ...
        postList.innerHTML = `
            <div class="empty-state text-center py-12">
                ...
                <p class="text-gray-500 mb-6">sessionStorage 방식으로 첫 번째 게시글을 작성해보세요.</p>
                <button onclick="window.location.href='/pages/sessionStorage/write'"
                        class="bg-orange-500 hover:bg-orange-700 text-white px-4 py-2 rounded-md transition duration-200">
                    글쓰기
                </button>
            </div>
        `;
        ...
        if (show) {
            postList.innerHTML = `
                <div class="loading-state text-center py-12">
                    ...
                    <p class="text-gray-500">sessionStorage에서 토큰을 확인하고 게시글을 불러오는 중...</p>
                </div>
            `;
        }
    }
    ...
    function clearAllTokens() {
        if (confirm('sessionStorage에서 모든 토큰을 삭제하시겠습니까?\\n삭제 후 로그인 페이지로 이동합니다.')) {
            removeTokens();
            showMessage('sessionStorage에서 모든 토큰이 삭제되었습니다.', 'success');
