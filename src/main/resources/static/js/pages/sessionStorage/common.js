...
// sessionStorage에 토큰 저장하기
function saveAccessToken(token) {
    sessionStorage.setItem('accessToken', token);
    console.log('sessionStorage에 Access Token 저장 완료');
}

function saveRefreshToken(token) {
    sessionStorage.setItem('refreshToken', token);
    console.log('sessionStorage에 Refresh Token 저장 완료');
}

// sessionStorage에서 토큰 삭제하기
function removeTokens() {
    sessionStorage.removeItem('accessToken');
    sessionStorage.removeItem('refreshToken');
    console.log('sessionStorage에서 모든 토큰 삭제 완료');
}
...
function updateUserInfo() {
    console.log('👤 sessionStorage 사용자 정보 업데이트 시작');
    const accessToken = getAccessToken();
    console.log('🔍 sessionStorage accessToken 확인:', accessToken ? '있음' : '없음');
    ...
    if (accessToken && isTokenValid(accessToken)) {
        console.log('sessionStorage 토큰 유효함');
        const user = getUserFromToken(accessToken);
        if (user) {
            console.log('sessionStorage 사용자 정보:', user.email);
            // 사용자 정보 표시
        ...
        console.log('sessionStorage 토큰 무효하거나 없음');
        // 로그인되지 않은 상태
        redirectToLogin();
    }
}
...
// 로그아웃 처리
function logout() {
    if (confirm('sessionStorage에서 토큰을 삭제하고 로그아웃하시겠습니까?')) {
        removeTokens();
        showMessage('sessionStorage에서 토큰이 삭제되었습니다. 로그아웃되었습니다.', 'success');
...
function redirectToLogin() {
    showMessage('sessionStorage에 유효한 토큰이 없습니다. 로그인 페이지로 이동합니다...', 'warning');
...
async function apiCall(url, options = {}) {
    ...
    if (accessToken) {
        ...
        console.log('sessionStorage에서 토큰 추가:', accessToken.substring(0, 20) + '...');
    }
