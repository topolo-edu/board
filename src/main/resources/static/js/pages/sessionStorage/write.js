// sessionStorage 글쓰기 페이지 JavaScript
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function testTokenExpiry() {
        ...
        if (!accessToken) {
            showMessage('sessionStorage에 토큰이 없습니다.', 'error');
            return;
        }
    ...
    function setupAutoSave() {
        ...
        function autoSave() {
            ...
            if (title || content) {
                sessionStorage.setItem('sessionStorage_draft_title', title);
                sessionStorage.setItem('sessionStorage_draft_content', content);
                console.log('sessionStorage에 임시 저장 완료');
            }
        }
    ...
    function restoreAutoSavedContent() {
        const savedTitle = sessionStorage.getItem('sessionStorage_draft_title');
        const savedContent = sessionStorage.getItem('sessionStorage_draft_content');
        ...
        if (savedTitle || savedContent) {
            showMessage('sessionStorage에서 임시 저장된 내용을 복원했습니다.', 'info');
        }
    }

    // 자동 저장된 내용 삭제
    function clearAutoSavedContent() {
        sessionStorage.removeItem('sessionStorage_draft_title');
        sessionStorage.removeItem('sessionStorage_draft_content');
        console.log('sessionStorage에서 임시 저장 내용 삭제 완료');
    }
});